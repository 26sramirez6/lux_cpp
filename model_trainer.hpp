...
#ifndef MODEL_TRAINER_HPP_
#define MODEL_TRAINER_HPP_
...
public:
  ...
  ModelTrainer(const HyperParameters &_hp, DQN &dqn_dynamic_, Args &&... args)
      : m_batch_size(_hp.m_replay_batch_size), m_vmin(_hp.m_nn_v_min),
        m_vmax(_hp.m_nn_v_max), m_atom_count(_hp.m_nn_atom_count),
        m_target_model_update(_hp.m_nn_target_model_update),
        m_gamma(std::pow(_hp.m_nn_gamma,_hp.m_nn_step_size)), m_lr(_hp.m_nn_lr),
        m_momentum(_hp.m_nn_momentum), m_weight_decay(_hp.m_nn_weight_decay),
        m_delta_z((_hp.m_nn_v_max - _hp.m_nn_v_min) /
                  (_hp.m_nn_atom_count - 1)),
        ...
        m_support(torch::linspace(_hp.m_nn_v_min, _hp.m_nn_v_max,
                                  _hp.m_nn_atom_count,
...
private:
  ...
  void inline computeTargetDistribution(const BatchType &_mini_batch) {
    ...
    std::cout << _mini_batch.m_reward << std::endl;
		...
		std::cout << _mini_batch.m_state.m_geometric.index({15,2,0,0});	

		std::cout << "next state: " << std::endl;
		std::cout << _mini_batch.m_next_state.m_geometric.index({15,2,0,0});	
